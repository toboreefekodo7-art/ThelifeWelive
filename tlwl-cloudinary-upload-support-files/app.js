...
function cloudinaryVideoPoster(info = {}) {
  if (info.thumbnail_url) return info.thumbnail_url;
  if (!info.secure_url || info.resource_type !== "video") return "";

  const urlWithoutQuery = info.secure_url.split("?")[0];
  const posterUrl = urlWithoutQuery.replace(/\.[a-z0-9]+$/i, ".jpg");
  return posterUrl.replace("/video/upload/", "/video/upload/so_0,w_1000,h_563,c_fill/");
}

function uploadLabel(info = {}, fallback) {
  const name = info.original_filename || info.public_id || fallback;
...
function applyUploadResult(form, kind, info) {
  if (kind === "video") {
    ...
    setUploadField(form, "video_thumbnail_url", cloudinaryVideoPoster(info));
    setUploadField(form, "video_resource_type", info.resource_type || "video");
    setUploadField(form, "video_format", info.format);
    setUploadField(form, "video_bytes", info.bytes);
    setUploadField(form, "video_duration", info.duration);
    setUploadStatus(form, `Video uploaded: ${uploadLabel(info, "Cloudinary video")}. You can submit when the rest of the form is complete.`);
    return;
  }

  setUploadField(form, "supporting_file_url", info.secure_url);
  setUploadField(form, "supporting_file_public_id", info.public_id);
  setUploadField(form, "supporting_file_thumbnail_url", info.thumbnail_url || info.secure_url);
  setUploadField(form, "supporting_file_type", info.resource_type || "auto");
  setUploadField(form, "supporting_file_format", info.format);
  setUploadField(form, "supporting_file_bytes", info.bytes);
  setUploadField(form, "supporting_file_duration", info.duration);
  setUploadStatus(form, `Supporting media uploaded: ${uploadLabel(info, "Cloudinary media")}. You can submit when the rest of the form is complete.`);
}
